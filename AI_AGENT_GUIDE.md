...
**① 8-bit 양자화 + SDPA (RTX 5060 8GB 기준 최적)**
- 4-bit NF4 → 8-bit INT8으로 교체: 역양자화 오버헤드 감소, 품질 향상
- `attn_implementation="sdpa"`: PyTorch 내장 최적화 어텐션 커널 사용 (flash-attn 설치 불필요)
- TF32 활성화: Blackwell 아키텍처에서 행렬 연산 가속
- VRAM: 4-bit(2.5GB) → 8-bit(~3.1GB) 사용, 추론 여유 ~2.7GB 확보

```
모드 비교 (RTX 5060 Laptop 8GB 기준):
  BF16   : 모델 ~6.2GB → 추론 여유 ~0.2GB (OOM 위험)
  8-bit  : 모델 ~3.1GB → 추론 여유 ~2.7GB ← 현재 선택
  4-bit  : 모델 ~2.5GB → 추론 여유 ~3.3GB (느림)
```
...
- **해결**: 프롬프트에 의존하지 않는 **출력 검증 가드(`looks_unnatural`)** 추가.
  ...
  여러 표현("오늘 출근 인원", "오늘 몇 명 출근", "출근 현황 보여줘")에서 모두 검증 완료
